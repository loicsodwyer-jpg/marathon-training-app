      ...
      {summaries.map((week) => (
        <div className="grid grid-cols-[42px_1fr_48px] items-center gap-3" key={week.weekNumber}>
          ...
          <div className="relative h-3 overflow-hidden rounded-full bg-stone-100 dark:bg-white/[0.08]">
            ...
            />
            <div className="absolute bottom-0 top-0 w-px bg-white/80 dark:bg-neutral-950" style={{ left: '85%' }} />
          </div>
          <p className="text-right text-xs font-semibold text-stone-700 dark:text-neutral-300">
            {formatPercent(week.completionPercent)}
          </p>
        </div>
      ))}
      <p className="text-xs leading-5 text-stone-500 dark:text-neutral-500">
